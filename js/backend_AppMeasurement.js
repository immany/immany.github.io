...
s.linkInternalFilters = 'javascript:,airreserve.net,dev2-rsv-web-cl.arg.x.recruit.co.jp,dev3-rsv-web-cl.arg.x.recruit.co.jp,dev4-rsv-web-cl.arg.x.recruit.co.jp,dev5-rsv-web-cl.arg.x.recruit.co.jp,connect.airregi.jp';
...
s.trackingServer="aa-metrics.airreserve.net"
...
s.siteID = 'airreserve';		// leftmost value in pagename
...
function s_doPlugins(s) {
	...
	var COOKIE_NAME_STORE_ID = 's_store_id';
	var COOKIE_NAME_CALENDAR_TYPE = 's_calendar_type';
	var COOKIE_NAME_BUSINESS_TYPE = 's_business_type';
	var COOKIE_NAME_INIT_PAGES_FLG = 's_init_flg';
	 ...
	 * 繝励Λ繝ｳ繧ｳ繝ｼ繝峨�蜿門ｾ� 
	 *****************************************/
	if (typeof(s_plan_cd) != 'undefined') {
		s.eVar35 = s_plan_cd;
	}
	
	/****************************************
	 * 繝ｦ繝ｼ繧ｶ繝ｼ縺ｫ邏蝉ｻ倥￥諠��ｱ繧辰ookie縺九ｉ蜿門ｾ�
	 ****************************************/
	var s_store_id = s.c_r(COOKIE_NAME_STORE_ID);
	var s_calendar_type = s.c_r(COOKIE_NAME_CALENDAR_TYPE);
	var s_business_type = s.c_r(COOKIE_NAME_BUSINESS_TYPE);
	
	var expires = new Date();
	expires.setTime(expires.getTime() + (365 * 24 * 60 * 60 * 1000) * 2);

	// StoreID
	s.prop4 = s_store_id;
	s.c_w(COOKIE_NAME_STORE_ID, s_store_id, expires);
	if (s.prop4) {
		s.eVar4 = 'D=c4';
	}
	
	// 繧ｫ繝ｬ繝ｳ繝繝ｼ繧ｿ繧､繝�
	s.eVar3 = s_calendar_type;
	s.c_w(COOKIE_NAME_CALENDAR_TYPE, s_calendar_type, expires);
	
	// 讌ｭ遞ｮ
	s.eVar5 = s_business_type;
	s.c_w(COOKIE_NAME_BUSINESS_TYPE, s_business_type, expires);
	
	/****************************************
	 * 蛻晄悄逋ｻ骭ｲ蟆守ｷ壹°縺ｩ縺�°繧貞叙蠕�
	 *****************************************/
	s.prop6 = (s.c_r(COOKIE_NAME_INIT_PAGES_FLG)) ? '蛻晄悄逋ｻ骭ｲ蟆守ｷ�' : '騾壼ｸｸ蟆守ｷ�';
	...
	if(splitPn[1] == 'reserve') {
		s.prop16 = 'reserve';
	} else if(splitPn[1] == 'petsalonboard') {
		s.prop16 = 'petsalonboard';
	}
	...
	if (s.eVar49) {
		...
		switch(s._channel) {
			...
			case'Referrers':
				...
				} else if (s._referringDomain.indexOf("search.yahoo.co.jp") > -1 && s.referrer == null) {
